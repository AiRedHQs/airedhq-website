...
const adSlots = [
  {
    id: "article-ad-1" as const,
    placement: "after-introduction" as const,
    provider: "adsense" as const,
    providerSlotId: "6093986231",
    enabled: true,
  },
  {
    id: "article-ad-2" as const,
    placement: "mid-article" as const,
    provider: "adsense" as const,
    providerSlotId: "6093986231",
    enabled: true,
  },
...
export function generateStaticParams() {
  return calculators.filter(({ published }) => published).map(({ slug }) => ({ slug }));
}
...
export default async function CalculatorPage({ params }: Props) {
  ...
  const related = calculators
    .filter(
      (item) => item.slug !== calculator.slug && item.category === calculator.category,
    )
    .slice(0, 4);
  const schema = [
    {
    ...
    >
      ...
      <section className="pb-16 pt-28 sm:pb-20 sm:pt-40">
        <Container className="max-w-[92rem]">
          <nav aria-label="Breadcrumb" className="text-xs text-white/38">
            <Link href="/">Home</Link>
            <span className="mx-2">/</span>
            <Link href="/calculators">Calculators</Link>
            <span className="mx-2">/</span>
            <span>{calculator.shortName}</span>
          </nav>
          <div className="mt-12 grid gap-10 lg:grid-cols-[1fr_.65fr] lg:items-end">
            <div>
              <div className="flex items-center gap-3 text-[color:var(--calculator-accent)]">
                <CalculatorIcon name={calculator.icon} className="size-5" />
                <span className="text-[10px] font-semibold uppercase tracking-[.22em]">
                  Free financial calculator
                </span>
              </div>
              <h1 className="mt-6 max-w-[13ch] text-[clamp(3.2rem,6.5vw,7rem)] font-semibold leading-[.9]">
                {calculator.name}
              </h1>
            </div>
            <p className="max-w-xl text-lg leading-8 text-white/52">
              {calculator.description} Change every input and see the estimate update
              immediately.
            </p>
          </div>
        </Container>
      ...
      <Container className="max-w-[92rem]">
        <FinanceCalculator calculator={calculator} />
        <ArticleAdSlot slot={adSlots[0]} />
      </Container>
            ...
            <div className="border-t border-white/10">
              {calculator.faqs.map((faq) => (
                <details key={faq.question} className="border-b border-white/10 py-6">
                  ...
                  <p className="pt-4 text-sm leading-7 text-white/50">{faq.answer}</p>
                </details>
              ))}
            </div>
          </div>
            ...
            <div className="mt-7 grid gap-px bg-white/10 sm:grid-cols-2 lg:grid-cols-4">
              {related.map((item) => (
                <Link
                  ...
                  className="group bg-[#07090c] p-6"
                >
                  <CalculatorIcon
                    name={item.icon}
                    className="size-5 text-[color:var(--calculator-accent)]"
                  />
                  <h3 className="mt-8 font-semibold">{item.name}</h3>
                  <ArrowRight className="mt-5 size-4 text-white/30 transition-transform group-hover:translate-x-1" />
                </Link>
              ))}
            </div>
          </div>
