...
const discoveryModes = [
  [Store, "Bazaars and businesses", "Explore trusted local places through structured profiles and useful context."],
  [Tags, "Nearby offers", "Find timely offers connected to places that are genuinely nearby."],
  [CalendarDays, "Events and fairs", "See what is happening around the city without searching across scattered sources."],
  [Compass, "Intent-aware discovery", "Move between food, shopping, services and experiences without losing local relevance."],
] as const;

export default function GoBazaarPreviewPage() {
  return (
    <main className="overflow-hidden bg-[#06101c] text-white">
        ...
        <Container className="relative flex min-h-[calc(100svh-4rem)] max-w-[96rem] items-center py-20">
          <div className="max-w-3xl">
            ...
            <h1 className="mt-6 text-[clamp(4rem,7vw,7.4rem)] font-semibold leading-[.9]">
              Your city has more<br />to <span className="text-[#2388ff]">discover.</span>
            </h1>
            <p className="mt-8 max-w-xl text-base leading-7 text-white/60">
              Find nearby bazaars, food, shopping, events and offers through one living map
              of local life.
            </p>
        ...
        <Container className="grid max-w-[96rem] gap-16 lg:grid-cols-[.7fr_1.3fr]">
          <div className="lg:sticky lg:top-28 lg:h-fit">
            ...
            <h2 className="mt-6 text-5xl font-semibold leading-[.95] sm:text-7xl">
              Local life is rich.<br />Finding it is <span className="text-[#2388ff]">fragmented.</span>
            </h2>
            ...
            {discoveryModes.map(([Icon, title, description]) => (
              <article key={title} className="grid gap-5 py-9 sm:grid-cols-[3rem_1fr]">
                ...
                <div>
                  ...
                  <p className="mt-3 max-w-xl text-sm leading-6 text-white/44">{description}</p>
                </div>
              </article>
            ))}
          </div>
        ...
        <Container className="relative flex min-h-[52rem] max-w-[96rem] items-center">
          <div className="max-w-lg">
            ...
            <div className="mt-10 flex flex-wrap gap-2">
              {["Food", "Shopping", "Street bazaars", "Fairs", "Events", "Services"].map((item) => (
                <span key={item} className="rounded-full border border-[#2388ff]/25 bg-[#2388ff]/8 px-4 py-2 text-xs text-[#75b8ff]">
                  {item}
                </span>
              ))}
            </div>
      ...
      <section className="py-28 sm:py-36">
        <Container className="max-w-[96rem]">
          ...
          <div className="mt-20 grid gap-px bg-white/10 md:grid-cols-3">
            {[
              ["Place confidence", "Location, category, operating status and merchant verification where available."],
              ["Timely relevance", "Offers and events carry visible dates instead of lingering indefinitely."],
      ...
      <section className="bg-[#081728] py-28 sm:py-36">
        <Container className="grid max-w-[96rem] gap-16 lg:grid-cols-[.74fr_1.26fr]">
          ...
          <div className="grid gap-px bg-white/10 sm:grid-cols-2">
            {[
              [MapPin, "Location integrity", "Clear location and service-area information."],
              [Store, "Merchant ownership", "Profiles that businesses can claim and maintain."],
              [CalendarDays, "Current information", "Dates and availability shown where relevant."],
              [ShieldCheck, "Visible verification", "Verified and unverified information are not presented alike."],
            ].map(([Icon, title, description]) => (
              <article key={title as string} className="bg-[#081728] p-7">
                <Icon className="size-6 text-[#3898ff]" />
                <h3 className="mt-10 text-xl font-semibold">{title as string}</h3>
                <p className="mt-3 text-sm leading-6 text-white/42">{description as string}</p>
              </article>
            ))}
          </div>
        </Container>
      ...
      <section className="py-12">
        <Container className="grid min-h-[34rem] max-w-[96rem] place-items-center bg-[radial-gradient(circle_at_50%_100%,rgba(35,136,255,.34),transparent_58%)] text-center">
          <div>
            <p className="text-[10px] font-semibold uppercase tracking-[0.24em] text-[#3898ff]">India-wide private beta</p>
